...
from .infrastructure.persistence.repositories import SqlAlchemyClienteRepository
...
def run(db: Session, inventario_path: str | None = None, pedidos_path: str | None = None) -> None:
    ...
    if ped_path.exists():
        ...
        for ped in pedidos:
            cliente = SqlAlchemyClienteRepository(db).upsert(ped.get("cliente", "Cliente"), ped.get("telefono", ""), "main")
